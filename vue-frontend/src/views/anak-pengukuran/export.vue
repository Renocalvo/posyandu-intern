...
/**
 * EXPORT EXCEL DENGAN KOMPRESI ZIP ✨
 * Fungsi utama untuk ekspor data ke file Excel terkompresi
 */
  ...
  try {
    ...
    ws.eachRow({ includeEmpty: false }, (row, rowNumber) => {
      ...
      row.eachCell((cell, col) => {
        // Kolom 2 (NIK) & 4 (TanggalUkur) sebagai teks
        if (col === 2 || col === 4) {
          const v = cell.value ?? ''
