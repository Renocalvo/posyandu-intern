...
const getAnomalyBadgeClass = (types) => {
  ...
  return types.some(t => criticalTypes.includes(t)) ? 'bg-danger' : 'bg-warning text-dark'
}
...
const showDataComparison = async (item) => {
  ...
  const diffColor = (val) => {
    ...
    const num = parseFloat(val)
    if (!Number.isFinite(num)) return 'text-muted'
    return num < 0 ? 'text-danger fw-bold' : num > 0 ? 'text-success fw-bold' : 'text-muted'
  }

  // Normalisasi tanggal ke format yyyy-mm-dd untuk input[type=date]
  const toInputDate = (v) => {
    const iso = fmtISO(v)
    return iso || ''
  }
  ...
  const result = await Swal.fire({
    title: '⚠️ Deteksi Anomali — Edit & Simpan',
    html: `
      <div class="text-start">
        <!-- Info Anak -->
        <div class="d-flex gap-4 mb-3 p-3 bg-light rounded">
          <div><span class="text-muted small">Nama Anak</span><br><strong>${item?.anak?.nama_anak || '-'}</strong></div>
          <div><span class="text-muted small">NIK</span><br><code>${item?.anak?.nik || '-'}</code></div>
        </div>

        <!-- Badge Anomali -->
        <div class="alert alert-warning py-2 mb-3">
          <strong>Anomali Terdeteksi:</strong>&nbsp;${anomalyBadges}
        </div>

        <!-- Tabel Perbandingan + Edit -->
            ...
            <thead class="table-dark">
              <tr>
                ...
                <th class="text-center" style="width:22%">
                  <span class="badge bg-secondary">Data Sebelumnya</span>
                </th>
                <th class="text-center" style="width:15%">Selisih (Δ)</th>
                <th class="text-center" style="width:45%">
                  <span class="badge bg-primary">Edit Nilai Terbaru</span>
                </th>
              </tr>
            </thead>
            <tbody>
              <!-- Tanggal Ukur -->
              <tr>
                <td class="fw-semibold text-muted small">Tanggal Ukur</td>
                <td class="text-center">${toDisplayDate(prev.tanggal_ukur)}</td>
                ...
                <td>
                  <input
                    ...
                    value="${toInputDate(curr.tanggal_ukur)}"
                  />
                </td>
              </tr>

              <!-- Tinggi -->
              <tr class="${diff.tinggi != null && diff.tinggi < -0.5 ? 'table-danger' : diff.tinggi != null && diff.tinggi > 5 ? 'table-warning' : ''}">
                <td class="fw-semibold text-muted small">Tinggi (cm)</td>
                <td class="text-center">${prev.tinggi ?? '-'}</td>
                <td class="text-center"><span class="${diffColor(diff.tinggi)}">${formatDiff(diff.tinggi)}</span></td>
                <td>
                  <div class="input-group input-group-sm">
                    ...
                    <button
                      ...
                      onclick="document.getElementById('edit-tinggi').value='${prev.tinggi ?? ''}'"
                      title="Pakai nilai sebelumnya"
                    >↩</button>
                  </div>
                </td>
              </tr>
              ...
              <!-- Berat -->
              <tr class="${diff.berat != null && diff.berat < -1 ? 'table-danger' : diff.berat != null && diff.berat > 3 ? 'table-warning' : ''}">
                <td class="fw-semibold text-muted small">Berat (kg)</td>
                <td class="text-center">${prev.berat ?? '-'}</td>
                <td class="text-center"><span class="${diffColor(diff.berat)}">${formatDiff(diff.berat)}</span></td>
                <td>
                  <div class="input-group input-group-sm">
                    <input
                      id="edit-berat"
                    ...
                    <button
                      type="button"
                      class="btn btn-outline-secondary btn-sm"
                      onclick="document.getElementById('edit-berat').value='${prev.berat ?? ''}'"
                      title="Pakai nilai sebelumnya"
                    >↩</button>
                  </div>
                </td>
              ...
              <!-- LILA -->
              <tr class="${diff.lila != null && diff.lila < -1 ? 'table-danger' : ''}">
                <td class="fw-semibold text-muted small">LILA (cm)</td>
                <td class="text-center">${prev.lila ?? '-'}</td>
                <td class="text-center"><span class="${diffColor(diff.lila)}">${formatDiff(diff.lila)}</span></td>
                <td>
                  <div class="input-group input-group-sm">
                    <input
                      id="edit-lila"
                    ...
                    <button
                      type="button"
                      class="btn btn-outline-secondary btn-sm"
                      onclick="document.getElementById('edit-lila').value='${prev.lila ?? ''}'"
                      title="Pakai nilai sebelumnya"
                    >↩</button>
                  </div>
                </td>
              ...
              <!-- Lingkar Kepala -->
              <tr class="${diff.lingkar_kepala != null && diff.lingkar_kepala < -0.5 ? 'table-danger' : diff.lingkar_kepala != null && diff.lingkar_kepala > 3 ? 'table-warning' : ''}">
                <td class="fw-semibold text-muted small">Lingkar Kepala (cm)</td>
                <td class="text-center">${prev.lingkar_kepala ?? '-'}</td>
                <td class="text-center"><span class="${diffColor(diff.lingkar_kepala)}">${formatDiff(diff.lingkar_kepala)}</span></td>
                <td>
                  <div class="input-group input-group-sm">
                    <input
                      id="edit-lk"
                      ...
                      min="0"
                      class="form-control"
                      value="${curr.lingkar_kepala ?? ''}"
                      placeholder="cm"
                    />
                    <button
                      type="button"
                      class="btn btn-outline-secondary btn-sm"
                      onclick="document.getElementById('edit-lk').value='${prev.lingkar_kepala ?? ''}'"
                      title="Pakai nilai sebelumnya"
                    >↩</button>
                  </div>
                </td>
        ...
        <div class="mt-3 p-2 bg-light rounded small text-muted">
          ...
          Edit nilai pada kolom <strong>Edit Nilai Terbaru</strong>, lalu klik <strong>Simpan</strong>.
          Tombol <strong>↩</strong> di tiap baris akan mengisi ulang nilai dari data sebelumnya.
        </div>
      </div>
    `,
    width: '860px',
    ...
    preConfirm: () => {
      ...
      const tinggi = document.getElementById('edit-tinggi')?.value
      const berat = document.getElementById('edit-berat')?.value
      const lila = document.getElementById('edit-lila')?.value
      const lk = document.getElementById('edit-lk')?.value

      // Validasi minimal tanggal harus diisi
      if (!tgl) {
        Swal.showValidationMessage('Tanggal Ukur tidak boleh kosong.')
        return false
      }

      return {
        tanggal_ukur: tgl,
        tinggi: tinggi !== '' ? parseFloat(tinggi) : curr.tinggi,
        berat: berat !== '' ? parseFloat(berat) : curr.berat,
        lila: lila !== '' ? parseFloat(lila) : curr.lila,
        lingkar_kepala: lk !== '' ? parseFloat(lk) : curr.lingkar_kepala,
      }
    }
  })

  if (result.isConfirmed && result.value) {
    await saveEditedData(item, result.value)
  }
}

/* =================== SAVE EDITED DATA =================== */
const saveEditedData = async (item, editedValues) => {
  try {
    const updatedData = {
      ...
      tanggal_ukur: editedValues.tanggal_ukur,
      tinggi: editedValues.tinggi,
      berat: editedValues.berat,
      lila: editedValues.lila,
      lingkar_kepala: editedValues.lingkar_kepala,
    }
...
const viewRiwayat = async (nik, nama = '') => {
  ...
  Swal.fire({
    ...
    html: `
      <div class="mb-2"><b>${nama || '-'}</b></div>
      <div class="small text-muted">NIK: <code>${nik || '-'}</code></div>
      <div class="mt-2">Memuat data riwayat…</div>
    `,
    ...
    const rowsHtml = list.map((r, i) => {
      ...
      return `
        <tr>
          ...
          <td>${asiBadges(r)}</td>
          <td>
            <span class="badge ${safeBool(r.kelas_ibu_balita_lama) ? 'bg-success' : 'bg-secondary'}">
    ...
    <div class="row">
      <div class="col-md-12">
        ...
        <div class="d-flex justify-content-between align-items-center mb-3 flex-wrap gap-2">
          <RouterLink
            :to="{ name: 'anak-pengukuran.create' }"
          ...
          <div class="d-flex align-items-center flex-wrap gap-2">
            <RouterLink
              :to="{ name: 'anak-pengukuran.import' }"
            ...
            <RouterLink
              :to="{ name: 'anak-pengukuran.export' }"
            ...
            <button
              v-if="dataFromSession"
              @click="resetCache"
              class="btn btn-md btn-info rounded d-inline-flex align-items-center"
            >
          ...
          <div class="card-body">
            <div class="row g-3 align-items-end">
              ...
              <div class="col-xl-4">
                ...
                <select
                  v-model="filterPosyandu"
                  class="form-select"
                  :class="{ 'text-placeholder': !filterPosyandu }"
                >
                  ...
                  <tbody>
                    ...
                    >
                      ...
                      <td>
                        <span
                          class="badge"
                          :class="fmtBool(getKelasIbu(item)) ? 'bg-success' : 'bg-secondary'"
                        >
                      ...
                      <td class="text-center">
                        <div class="d-flex justify-content-start flex-wrap gap-2">
                          ...
                          <button
                            ...
                            class="btn btn-sm btn-secondary rounded-sm shadow border-0"
                          >
                            RIWAYAT
                          </button>
...
.table-riwayat td .badge { font-weight: 600; }
...
.swal2-popup .input-group-sm .btn {
